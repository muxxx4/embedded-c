...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  result = HAL_UART_Receive(&huart3, receiveBuff, 1, 10);

	  if (result == HAL_OK) {
		  switch (receiveBuff[0]) {
		  	  case 'b':
		  		  uint8_t state15 = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_15);

		  		  if (state15 == GPIO_PIN_RESET) {
		  	  ...
		  	  case 'r':
		  		  uint8_t state14 = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_14);

		  		  if (state14 == GPIO_PIN_RESET) {
		  	  ...
		  	  case 'o':
		  		  uint8_t state13 = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_13);

		  		  if (state13 == GPIO_PIN_RESET) {
		  	  ...
		  	  case 'g':
		  		  uint8_t state12 = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_12);

		  		  if (state12 == GPIO_PIN_RESET) {
